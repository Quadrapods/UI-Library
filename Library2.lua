...
do
    ...
    local function ismouseover(obj)
        ...
        if
            services.InputService:GetMouseLocation().X >= posX and services.InputService:GetMouseLocation().Y >= posY and
                services.InputService:GetMouseLocation().X <= sizeX and
                services.InputService:GetMouseLocation().Y <= sizeY
         then
            return true
        end
...
function Library.CreateColorPicker(default, parent, count, flag, callback)
    ...
    local function set(color, nopos)
        ...
        if not nopos then
            saturationpicker.Position =
                UDim2.new(
                0,
                (math.clamp(sat * saturation.AbsoluteSize.X, 0, saturation.AbsoluteSize.X - 4)),
                0,
                (math.clamp((1 - val) * saturation.AbsoluteSize.Y, 0, saturation.AbsoluteSize.Y - 4))
            )
            huepicker.Position =
                UDim2.new(0, 0, 0, math.clamp((1 - hue) * saturation.AbsoluteSize.Y, 0, saturation.AbsoluteSize.Y - 4))
        end
    ...
    local function updatehue(input)
        local sizeY =
            1 - math.clamp(((input.Position.Y - hueframe.AbsolutePosition.Y) + 36) / hueframe.AbsoluteSize.Y, 0, 1)
        local posY =
            math.clamp(
    ...
    function windowtypes:Tab(name)
        ...
        function tabtypes:Section(options)
            ...
            function sectiontypes:Toggle(options)
                ...
                holder.MouseButton1Down:Connect(
                    function()
                        if not toggled then
